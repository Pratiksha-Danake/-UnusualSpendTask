...
public class CreditCardManager {
    ...
    public List<UnusualSpend> getListOfUnusualSpendCustomer() {
            ...
            for (Transaction previous : previousMonthTransactionList){
                if (current.getCategory().equals(previous.getCategory()) && current.getAmountSpend() >= (1.5*previous.getAmountSpend())){
                    unusualSpendCustomerList.add(new UnusualSpend(current.getCategory(),
                            current.getAmountSpend() - previous.getAmountSpend(),current.getCustomerId()));
                }
            }
        }
    ...
    public void sendEmailAlert(List<UnusualSpend> unusualSpendCustomerList, List<Customer> customerList) {
        ...
        spendByCustomer.forEach((customerId, spends) -> {
            ...
            String name = customerManager.getCustomerById(customerId).getName();
            String email = customerManager.getCustomerById(customerId).getEmail();

            EmailService.sendEmail(customerId,name,email,overallUnusualSpend, new String(stringBuilder));
        });
    }
}
